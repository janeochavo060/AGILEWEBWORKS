...
module.exports.preBuildDevelopment = async () => {
  ...
  // const imageHandler = await axios.get(
  //   envVars.NEXT_PUBLIC_TENANT_API +
  //     "/api/route/%2F?include=blockContents.block"
  // );
  // const page = dataFormatter.deserialize(imageHandler.data);
  // const mainBanner = page?.blockContents?.find(
  //   (e) => e.block.component === "MainBanner"
  // );
  // const imageUrl = mainBanner?.data?.main?.images[0];

  // if (imageUrl) {
  //   const file = fs.createWriteStream("./public/default-image.webp");
