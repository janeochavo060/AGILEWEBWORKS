...
module.exports.preBuildDevelopment = async () => {
  ...
  const footer = await axios.get(
    envVars.NEXT_PUBLIC_TENANT_API + "/api/globals/footer"
  );
  const footerDetails = dataFormatter.deserialize(footer.data);
  ...
  generateStaticJson("footer.json", footerDetails);
