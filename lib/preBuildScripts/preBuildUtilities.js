...
module.exports.preBuildDevelopment = async() => {
  ...
  // Form
  const formHandler = await axios.get(envVars.NEXT_PUBLIC_TENANT_API + '/api/forms/get-in-touch?include=blueprint');
  ...
  // Menu Data
  const menusHandler = await axios.get(envVars.NEXT_PUBLIC_TENANT_API + '/api/menus/header?include=nodes.children,parentNodes');
